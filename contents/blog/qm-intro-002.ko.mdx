...
기저 벡터의 이동뿐 아니라, 격자 전체가 어떻게 변형되는지도 확인해 보자. 아래 시각화에서 $\hat{i}$는 제자리에 머물고, $\hat{j}$만 기울어지면서 격자가 오른쪽으로 기울어지는 전단 효과를 볼 수 있다.
...
격자 전체에 이 변환을 적용하면, 2차원 격자가 하나의 직선으로 납작해지는 모습을 확인할 수 있다.

```plot3d
type: transform2d
title: 차원 축소 — 2차원이 1차원으로
matrix: 2, -1, 4, -2
grid: -2, 2
step: 1
```
